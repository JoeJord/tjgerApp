...
public class PartsDlg extends HGBaseConfigDialog implements OnSharedPreferenceChangeListener {
    ...
    protected void createComponents() {
        ...
        for (int i=0; i<cardSetTypes.length; i++) {
            String cardType = cardSetTypes[i];
        ...
        for (int i=0; i<userPartSets.length; i++) {
            addComboBox(userPartSets[i], config.getPartSets(userPartSets[i]), config.getActivePartSet(userPartSets[i]), cbList, false);
        }
        for (int i=0; i<userParts.length; i++) {
            addComboBox(userParts[i], config.getParts(userParts[i]), config.getActivePart(userParts[i]), cbList, false);
        }
        // get the color choosers
        for (int i=0; i<colorTypes.length; i++) {
            String colorType = colorTypes[i];
        ...
        configComboList = cbList.toArray(new PartsComboBox[cbList.size()]);
        configColorList = ccList.toArray(new HGBaseColorPreference[ccList.size()]);
    ...
    private void changeArrangement() {
		...
		if (a != null) {
            ...
            for (int i=0; i<cardSetTypes.length; i++) {
                changePartOfArrangement(indexOf(cardSetTypes[i], true), a.getCardSet(cardSetTypes[i]));
            }
            for (int i=0; i<userParts.length; i++) {
                changePartOfArrangement(indexOf(userParts[i], false), a.getPart(userParts[i]));
            }
            for (int i=0; i<userPartSets.length; i++) {
                changePartOfArrangement(indexOf(userPartSets[i], false), a.getPartSet(userPartSets[i]));
            }
    ...
    protected void testChangedArrangement(Arrangement arrange) {
        if (!onChangeArrangement && indexOf(ConstantValue.CONFIG_ARRANGEMENT, true) >= 0
                && !isGivenArrangement(arrange)) {
            GameConfig config = GameConfig.getInstance();
            int selIndex = -1;
    ...
    private boolean isGivenArrangement(Arrangement arrange) {
        ...
        if (!isPartOk(back, indexOf(ConstantValue.CONFIG_BACKGROUND, true)) ||
            !isPartOk(board, indexOf(ConstantValue.CONFIG_BOARD, true)) ||
            !isPartOk(piece, indexOf(ConstantValue.CONFIG_PIECESET, true)) ||
            !isPartOk(cover, indexOf(ConstantValue.CONFIG_COVER, true)) /*||
            !isPartOk(cards, indexOf(ConstantValue.CONFIG_CARDSET, true))*/ ) {
            return false;
        } else {
            // test for the card sets
            for (int i=0; i<cardSetTypes.length; i++) {
                CardSet cards = arrange.getCardSet(cardSetTypes[i]);
                if (!isPartOk(cards, indexOf(cardSetTypes[i], true))) {
                    return false;
                }
            }
            // test the user defined parts and part sets
            for (int i=0; i<userParts.length; i++) {
                if (!isPartOk(arrange.getPart(userParts[i]), indexOf(userParts[i], false))) {
                    return false;
                }
            }
            for (int i=0; i<userPartSets.length; i++) {
                if (!isPartOk(arrange.getPartSet(userPartSets[i]), indexOf(userPartSets[i], false))) {
                    return false;
                }
            }
            // test the colors
    ...
    /**
     * @param p A part of the arrangement.
    ...
    private boolean isPartOk(Part arrangementPart, int index) {
        if (arrangementPart!=null && index>0 && !arrangementPart.equals(configComboList[index].getSelectedItem())) {
            return false;
        } else {
            return true;
        }
