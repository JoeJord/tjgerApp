...
final public class PlayerManager {
	...
	public String getPieceColor(int i) {
		String pieceConfigKey = (i == HUMAN_PLAYER_INDEX) ? ConstantValue.CONFIG_PLAYERPIECECOLOR 
														  : ConstantValue.CONFIG_PLAYERPIECECOLOR + i;				
    ...
    public String getPlayerName(int i) {
        ...
        if (!HGBaseTools.hasContent(playerName) || (i == HUMAN_PLAYER_INDEX && (playerName.equals(getDefaultHumanPlayerName()) 
        	...
        	return (i == HUMAN_PLAYER_INDEX && isOnlyOneHuman) ? getDefaultHumanPlayerName()
    ...
    public int getIndexOfPlayer(GamePlayer player) {
        ...
        for (int i=0; i<playerList.length && index==-1; i++) {
            if (playerList[i].equals(player)) index=i;
        }
        return index;
    }
