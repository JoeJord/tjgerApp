...
public class MainPanel extends Fragment {
	...
	protected void setZoomFitWindowDelayed(final boolean onCreate) {
		if (scrollPane != null && gamePanel != null) {
			...
			HGBaseTools.delay(ZOOM_DELAY_TIME, new UpdateUiTimerTask(getMainFrame(), new Runnable() {
     ...
     * 
     * @param the parent view to add the new created scroll view
	...
	private void setNewStatePanelPosition(String position) {
	    ...
	    if (StatePanelPosition.statepanel_auto.name().equals(position)) {
	    	position = HGBaseGuiTools.isScreenPortrait(getMainFrame())? StatePanelPosition.statepanel_bottom.name()
	    													     : StatePanelPosition.statepanel_right.name();
	    }
		if (!StatePanelPosition.statepanel_none.name().equals(position)) {
			int index = (StatePanelPosition.statepanel_bottom.name().equals(position) || 
						 StatePanelPosition.statepanel_right.name().equals(position)) ? 1 
								 										              : 0;
			int layout = (StatePanelPosition.statepanel_bottom.name().equals(position) || 
			      	  	  StatePanelPosition.statepanel_top.name().equals(position)) ? LinearLayout.VERTICAL 
			      	  			  													 : LinearLayout.HORIZONTAL;
	        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 
	        															 statePanel.getPanelHeight());
	...
	protected Button createNextGameButton() {
		Button nextGame = HGBaseGuiTools.createButton(getMainFrame(), HGBaseText.getText("game_next"), new OnClickListener() {
	...
	protected void addNextGameButtonToRoot(View button, ButtonPosition pos) {
        ...
        if (pos != ButtonPosition.BOTTOM && pos != ButtonPosition.TOP 
        		&& !(pos == ButtonPosition.CENTER && HGBaseGuiTools.isScreenPortrait(getMainFrame()))) {
        	lp.width = HGBaseGuiTools.getScreenSize(getActivity()).x / 2;       	
        }
        if (pos == ButtonPosition.TOP || pos == ButtonPosition.TOP_LEFT 
        		|| pos == ButtonPosition.TOP_CENTER || pos == ButtonPosition.TOP_RIGHT) {
